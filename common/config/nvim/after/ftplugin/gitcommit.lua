-- ref: https://raw.githubusercontent.com/ryoppippi/dotfiles/refs/heads/main/nvim/after/ftplugin/gitcommit.lua
...
vim.schedule(function()
    if to_bool(vim.fn.executable(pre_commit_path)) then
        ...
        vim.fn.termopen("cd " .. git_root .. " && " .. pre_commit_path, {
            ---@param _job_id number  ID of the finished job
            ---@param exit_code number  Exit code of the process
            ---@param _event_type string  Event Type (fixed as "exit")
            on_exit = function(_job_id, exit_code, _event_type)
                if exit_code ~= 0 then
                    vim.notify("pre-commit failed", vim.log.levels.ERROR)
                else
                    vim.notify("pre-commit passed", vim.log.levels.INFO)
                end
...
vim.api.nvim_create_autocmd("BufWritePost", {
    buffer = commit_bufnr,
    callback = function()
        if not to_bool(vim.b.is_pre_commit_pass) then
            local err_msg = "Please wait for pre-commit hook."
            vim.notify(err_msg, vim.log.levels.ERROR)
            vim.api.nvim_err_writeln(err_msg)
        end

        if to_bool(vim.fn.executable(commit_msg_path)) then
            ...
            vim.fn.jobstart(commit_msg_path .. " " .. abs_buffer_filename, {
                ...
                on_exit = function(_job_id, exit_code, _event_type)
                    if exit_code ~= 0 then
                        ...
                        vim.notify("commit-msg hook passed", vim.log.levels.INFO)
                    end
