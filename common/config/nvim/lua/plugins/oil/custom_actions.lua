...
local function listWeztermPanes()
    ...
    local panes_list = vim.iter(json):map(lambda("obj: {pane_id = obj.pane_id, tab_id = obj.tab_id }"))
    return panes_list
end
...
local function getNeovimPaneIdFromWezterm()
    ...
    if not wezterm_pane_id then
        vim.notify("Wezterm pane not foun", vim.log.levels.ERROR)
        return
    end
...
local function ensureWeztermPreviewPane()
    ...
    callback = function()
        ...
        local updateWeztermPreview = debounce(
            vim.schedule_wrap(function()
                ...
                local cursor_entry = oil.get_cursor_entry()
                -- Don't update in visual mode. Visula mode implies editing not browsing,
